...
def play(self): #функция-основной цикл игры
    print("Добро пожаловать в Перудо!")
    print("Введите ваше имя: ") #устанавливаем первого игрока
    self.username = input().strip()
    print("Кто делает первый ход? Введите имя: ") #устанавливаем очередность ходов
    first = input()
    ...
    while len(self.get_active_players()) > 1: #пока у нас больше чем 1 активный игрок
        ...
        while True:
            idxidx = idx % len(real_players) #крутим игроков по кругу. Например, если игроков 4, то очередность у программы будет 0, 1, 2, 3, 0, 1, и тд
            ...
            maputo_active = any(p.status == "мапуто" for p in self.get_active_players()) #статусы из функции про статусы))
            prob = formula(k, val, user.hand, self.other_dice(), maputo=maputo_active) #посчитанная через функцию формулы вероятность
            print(f"Вероятность утверждения {real.name}: {round(prob * 100, 2)}%") #выводим переведенную в проценты (с округлением)
...
"""  
print("Игра Перудо")
print("Введите имена игроков через пробел: ") #самые-самые первые строки, которые выводит код
names = input().split() 
